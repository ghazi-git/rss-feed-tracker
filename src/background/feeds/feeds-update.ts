 ...
 * - Update feed run times if the update frequency changes
 */
export async function updateFeed(id: number, feedData: UpdateFeedFormData) {
  ...
  if (updated.feed.updateFrequency !== old.feed.updateFrequency) {
    ...
    if (updated.feed.updateFrequency) {
      nextRunAt = updated.feed.lastRunAt
        ? updated.feed.lastRunAt + updated.feed.updateFrequency
        : Date.now();
    }
    updated.feed.nextRunAt = nextRunAt;
  }
  await nodeStore.put(updated);

  if (newParentId && newParentId !== old.parentId && updated.unreadCount) {
